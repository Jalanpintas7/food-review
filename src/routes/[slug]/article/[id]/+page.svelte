<script>
  ...
  // Untuk kompatibilitas dengan kode lama, kita masih menggunakan struktur allArticleData
  // tapi ini hanya untuk referensi dan akan dihapus nanti
  ...
  // Fungsi untuk merender konten JSON dari database
  function renderContent(contentArray) {
    ...
    return contentArray.map(block => {
      switch (block.type) {
        ...
        case 'heading':
          return `<h${block.level || 2} class="text-2xl font-bold text-gray-800 mb-4 mt-8">${block.content}</h${block.level || 2}>`;
        case 'image':
          return `
            <figure class="my-6">
              <img src="${block.url}" alt="${block.alt || ''}" class="w-full rounded-lg shadow-md">
              ${block.caption ? `<figcaption class="text-sm text-gray-600 mt-2 text-center">${block.caption}</figcaption>` : ''}
            </figure>
          `;
        case 'quote':
          return `
            <blockquote class="border-l-4 border-primary-500 pl-4 py-2 my-6 italic text-gray-700">
              <p>${block.content}</p>
              ${block.source ? `<cite class="text-sm text-gray-600 mt-2 block">— ${block.source}</cite>` : ''}
            </blockquote>
          `;
        default:
          return block.content || '';
          ...
          <section class="mt-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {#each relatedArticles as relatedArticle}
                <a href="/{websiteSlug}/article/{relatedArticle.id}" class="group">
